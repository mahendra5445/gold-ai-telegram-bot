...
from config import effective_decimals, ASSETS, ASSET_LIST
...
from trade_tracker import has_open_trade, save_trade, get_all_trades
import guards
...
async def _check_asset(application, asset: str) -> None:
    ...
    if live_price is not None:
        # Price ke hisaab se precision — coin sasta ho gaya to config ka
        # hardcoded `decimals` SL/TP ko rounding mein hi kha jaata hai.
        decimals = effective_decimals(live_price, decimals)
    ...
    async with trade_lock:
        ...
        # Risk guardrails: max open trades, daily signal cap, consecutive
        # loss auto-pause. Trade banane se PEHLE.
        allowed, why = guards.check(get_all_trades())
        if not allowed:
            logger.info(f"[GUARD] {asset.upper()} signal blocked — {why}")
            return
